...
static data3000BCto3000AD = [ //https://ssd.jpl.nasa.gov/txt/p_elem_t2.txt
    ...
    //From Meeus, CH7
    static julainDateToGregorian(jd){
        let temp=jd+.5;
        let Z=Math.trunc(temp);
        let F=temp-Z;
        let A=Z;
        if(Z>=2299161){
            let alpha=this.INT((Z-1867216.25)/36524.25);
            A=Z+1+alpha-this.INT(alpha/4);
        }
        let B=A+1524;
        let C=this.INT((B-122.1)/365.25);
        let D=this.INT(365.25*C);
        let E=this.INT((B-D)/30.6001);

        let day=B-D-this.INT(30.6001*E)+F;
        let month=E-1;
        if(E>13){
            month=E-13;
        }
